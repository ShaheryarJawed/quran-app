...
// --- AUTH LOGIC ---
// Managed by shared_auth.js
// Global initAuth() and signOut() are available.

// ... features (saveItem) rely on currentUser which is in shared_auth.js
// Wait, shared_auth.js defines const currentUser.
// If script.js uses it, it's fine as long as shared_auth runs first.
// But const in global scope is not window.currentUser. It's block scoped if module? No, script tags are global.
// However `const` doesn't attach to window.
// I should rely on localStorage in saveItem or just use the global if available.

// To be safe, I will change saveItem to read from localStorage directly or assume global.
// Let's just remove the definitions here.
...
async function initApp() {
    ...
    // 9. Premium Dashboard Features (Auth Only)
    safeRun("updatePremiumDashboard", updatePremiumDashboard);
}

// --- PREMIUM DASHBOARD FEATURES ---
function updatePremiumDashboard() {
    const user = JSON.parse(localStorage.getItem('currentUser'));
    if (!user) return; // Keep hidden if guest
    ...
    if (lastRead) {
        if (document.getElementById('resume-surah-name')) {
            document.getElementById('resume-surah-name').textContent = lastRead.surahName || "Surah Al-Fatiha";
        }
        if (document.getElementById('resume-ayah-num')) {
            document.getElementById('resume-ayah-num').textContent = `Ayah ${lastRead.ayahNum || 1}`;
        }
        const pct = lastRead.progress || 0;
        if (document.getElementById('resume-progress')) {
            document.getElementById('resume-progress').style.width = `${pct}%`;
        }
    }

    // 3. Vault Stats (Compact Card)
    updateStreak();
    const favorites = JSON.parse(localStorage.getItem('user_favorites') || '[]');
    const favCountEl = document.getElementById('vault-favs-count');
    if (favCountEl) favCountEl.textContent = favorites.length;
}

function updateStreak() {
    let streakData = JSON.parse(localStorage.getItem('user_streak') || '{"count": 0, "lastDate": null}');
    ...
    if (streakData.lastDate !== today) {
        ...
        if (streakData.lastDate) {
            ...
            if (diffDays === 1) {
                ...
                newCount = streakData.count; // Same day (prevents re-increment on same day)
            }
        }

        streakData = { count: newCount, lastDate: today };
        localStorage.setItem('user_streak', JSON.stringify(streakData));
    }

    // Update both UI locations
    const countEl = document.getElementById('streak-count');
    const vaultCountEl = document.getElementById('vault-streak-count');

    if (countEl) countEl.textContent = streakData.count;
    if (vaultCountEl) vaultCountEl.textContent = streakData.count;
}

window.resumeLastSession = function () {
    const lastRead = JSON.parse(localStorage.getItem('user_last_read'));
    if (lastRead) {
        window.location.href = `quran.html?surah=${lastRead.surahId}&ayah=${lastRead.ayahNum}`;
    } else {
        window.location.href = 'quran.html';
    }
};

window.openProfileVault = function () {
...
async function fetchAllData() {
    ...
    // 1. Check if DASHBOARD_DATA already exists (from dashboard_data.js)
    if (window.DASHBOARD_DATA && window.DASHBOARD_DATA.daily) {
        console.log("Using local static data source.");
        return; // Success! Data is already there.
    }

    // 2. Fallback to API (Only if local data missing)
    console.log("Local data missing, trying API...");
    ...
    try {
        ...
        window.DASHBOARD_DATA = {
            daily: [daily], // API return single object, wrap in array
            courses,
            topics,
            duas,
            seerah
        ...
        console.warn("API Fetch Failed, and no local data found.", e);
        // Do NOT throw fatal error, let UI show skeletons or empty states
        // throw e; 
    }
}
...
function renderDailyFeed() {
    ...
    if (ayahBody) {
        ayahBody.innerHTML = `
            <div class="ar-text" style="font-family:'Amiri', serif; font-size:1.5rem; text-align:center; color:#1e293b; margin-bottom:0.5rem; padding-top:1rem;">${content.ayah.text}</div>
    ...
    if (hadithBody) {
        hadithBody.innerHTML = `
             <div style="position:absolute; top:1rem; right:1rem; cursor:pointer; color:#cbd5e1;" onclick="saveItem('Hadith', {en: '${content.hadith.text.replace(/'/g, "\\'")}'})">
                <i class='bx bx-heart' style="font-size:1.5rem;"></i>
            </div>
            <div class="en-text" style="font-style:italic; font-size:1rem; color:#334155; line-height:1.6; text-align:center; padding-top:1rem;">"${content.hadith.text}"</div>
        `;
    ...
    if (duaBody) {
        duaBody.innerHTML = `
             <div style="position:absolute; top:1rem; right:1rem; cursor:pointer; color:#cbd5e1;" onclick="saveItem('Dua', {ar: '${content.dua.text.replace(/'/g, "\\'")}', en: '${content.dua.trans.replace(/'/g, "\\'")}'})">
                <i class='bx bx-heart' style="font-size:1.5rem;"></i>
            </div>
            <div class="ar-text" style="font-family:'Amiri', serif; font-size:1.5rem; color:#1e293b; text-align:center; margin-bottom:0.5rem; padding-top:1rem;">${content.dua.text}</div>
